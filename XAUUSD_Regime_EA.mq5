...
input double               InpMaxTrendAdxDrop          = 0.00;
input double               InpLongTrendAdxOffset       = 8.0;
input double               InpLongTrendEfficiencyOffset = 0.12;
input double               InpLongTrendDiGapOffset     = 5.0;
...
input int                  InpMinRangeBoundaryTouches  = 2;
input double               InpRangeTouchBufferAtr      = 0.20;
input bool                 InpRequireRangeFalseBreak   = true;
input double               InpRangeFalseBreakBufferAtr = 0.02;
...
input bool                 InpUseSignalBarTrendEntry   = false;
...
input bool                 InpUseEntryEmaAlignment     = false;
input int                  InpEntryFastEmaPeriod       = 8;
input int                  InpEntrySlowEmaPeriod       = 21;
...
input double               InpTrendBuyMaxRsi           = 66.0;
input double               InpTrendSellMinRsi          = 28.0;
input bool                 InpUseCandleTrendFilter     = true;
input double               InpCandleMinBodyAtr         = 0.10;
input double               InpCandleMaxOppositeWickRatio = 0.45;
input int                  InpCandleMomentumLookback   = 3;
input int                  InpCandleMinMomentumCloses  = 2;
input bool                 InpUseStructureRoomFilter   = false;
input bool                 InpUseStructurePullbackEntry = false;
input int                  InpStructureLookbackBars     = 48;
input int                  InpStructurePivotSpan        = 2;
input double               InpMinRoomToStructureAtr    = 0.35;
input double               InpStructureBreakBufferAtr  = 0.03;
input double               InpStructurePullbackZoneAtr = 0.35;
input double               InpStructureRejectBufferAtr = 0.05;
input bool                 InpUseFibonacciTrendFilter  = false;
input bool                 InpUseFibonacciForLong      = true;
input bool                 InpUseFibonacciForShort     = false;
input int                  InpFibonacciLookbackBars    = 96;
input double               InpFibonacciMinImpulseAtr   = 1.20;
input double               InpFibonacciMinRetracement  = 0.236;
input double               InpFibonacciMaxRetracement  = 0.886;
input double               InpFibonacciZoneBufferAtr   = 0.25;

input bool                 InpUseWiderTrendStop        = false;
input double               InpTrendAtrStopMultiplier   = 1.20;
input double               InpRangeAtrStopBuffer       = 0.25;
input double               InpTrendRewardRisk          = 2.90;
input double               InpLongTrendRewardRisk      = 1.70;
input double               InpShortTrendRewardRisk     = 0.00;
input double               InpRangeRewardRisk          = 1.00;
input ENUM_RANGE_TARGET_MODE InpRangeTargetMode        = RANGE_TARGET_NEAREST;
input ENUM_RANGE_STOP_MODE InpRangeStopMode            = RANGE_STOP_ATR;
...
input double               InpMinRangeTradeR           = 0.75;

input bool                 InpUseRangeGrid             = true;
input int                  InpRangeGridMaxPositions    = 2;
input double               InpRangeGridStepAtr         = 0.90;
input double               InpRangeGridTakeProfitAtr   = 1.20;
input double               InpRangeGridStopBufferAtr   = 0.25;
input double               InpRangeGridMaxWidthAtr     = 3.20;
input double               InpRangeGridMinChoppiness   = 62.0;
input double               InpRangeGridMaxEfficiency   = 0.18;
input double               InpRangeGridEntryZone       = 0.35;
input int                  InpRangeGridMinTouches      = 2;
input bool                 InpRangeGridUseHigherBias   = true;
...
input bool                 InpAllowRecoveryTradeAfterDailyLoss = true;
input int                  InpMaxDailyRecoveryTrades   = 1;
input double               InpMinDailyLossForRecoveryTrade = 7.50;
input int                  InpMinRecoveryLossHoldMinutes = 40;
input int                  InpLossCooldownThreshold    = 2;
input int                  InpLossCooldownBars         = 4;
input bool                 InpUseBreakevenStop         = true;
input double               InpBreakevenTriggerR        = 0.40;
input int                  InpBreakevenBufferPoints    = 30;
input bool                 InpUseAtrTrailingStop       = true;
input double               InpAtrTrailTriggerR         = 1.10;
...
input double               InpMaxLossCloseR            = 0.90;
input bool                 InpUseTradeHours            = false;
input int                  InpTradeStartHour           = 4;
input int                  InpTradeEndHour             = 23;
input bool                 InpAvoidWeekendEntries      = false;
input int                  InpFridayLastEntryHour      = 20;
...
int      g_last_loss_hold_seconds = 0;
...
int g_fast_ema_entry_handle = INVALID_HANDLE;
int g_slow_ema_entry_handle = INVALID_HANDLE;
...
void OnTick()
{
   ...
   if(is_new_signal_bar && InpUseRangeGrid && snapshot.regime == REGIME_RANGE &&
      InpStrategyMode != STRATEGY_TREND_ONLY)
   {
      ManageRangeGrid(snapshot);
      RefreshRiskStats();
      UpdatePanel();
      return;
   }
...
bool CreateIndicatorHandles()
{
   ...
   g_fast_ema_entry_handle = iMA(g_symbol, InpEntryTimeframe, InpEntryFastEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   g_slow_ema_entry_handle = iMA(g_symbol, InpEntryTimeframe, InpEntrySlowEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   ...
   if(g_fast_ema_handle == INVALID_HANDLE || g_slow_ema_handle == INVALID_HANDLE ||
       ...
       g_fast_ema_entry_handle == INVALID_HANDLE || g_slow_ema_entry_handle == INVALID_HANDLE ||
...
void ReleaseIndicatorHandles()
{
   ...
   if(g_fast_ema_entry_handle != INVALID_HANDLE) IndicatorRelease(g_fast_ema_entry_handle);
   if(g_slow_ema_entry_handle != INVALID_HANDLE) IndicatorRelease(g_slow_ema_entry_handle);
...
bool BuildStrategySnapshot(StrategySnapshot &snapshot)
{
   ...
   bool adx_not_fading = InpMaxTrendAdxDrop <= 0.0 || adx[1] >= adx[3] - InpMaxTrendAdxDrop;
   ...
   else if(snapshot.regime == REGIME_RANGE)
   {
      ...
      double touch_buffer = atr[1] * MathMax(0.0, InpRangeTouchBufferAtr);
      double false_break_buffer = atr[1] * MathMax(0.0, InpRangeFalseBreakBufferAtr);
      int lower_touches = CountLowerBoundaryTouches(rates, 2, range_lookback, range_low, touch_buffer);
      int upper_touches = CountUpperBoundaryTouches(rates, 2, range_lookback, range_high, touch_buffer);
      bool range_structure_ok = lower_touches >= InpMinRangeBoundaryTouches &&
                                upper_touches >= InpMinRangeBoundaryTouches;
      ...
      bool near_upper = rates[1].high >= range_high - edge_buffer &&
                        ...
                        rates[1].close >= bands_mid[1];
      bool buy_false_break = !InpRequireRangeFalseBreak ||
                             (rates[1].low <= range_low - false_break_buffer &&
                              rates[1].close > range_low + close_back);
      bool sell_false_break = !InpRequireRangeFalseBreak ||
                              (rates[1].high >= range_high + false_break_buffer &&
                               rates[1].close < range_high - close_back);
      ...
      if(range_structure_ok && near_lower && buy_false_break && rsi_turn_up && bullish_reject && buy_vwap_ok && buy_sweep_ok && buy_higher_ok)
      {
         signal = SIGNAL_BUY;
         reason = "Range buy v3: tested support false-break reversal";
      }
      else if(range_structure_ok && near_upper && sell_false_break && rsi_turn_down && bearish_reject && sell_vwap_ok && sell_sweep_ok && sell_higher_ok)
      {
         signal = SIGNAL_SELL;
         reason = "Range sell v3: tested resistance false-break reversal";
      }
      else
      {
...
bool BuildIntrabarTrendSignal(StrategySnapshot &snapshot, bool is_new_entry_bar)
{
   ...
   double entry_ema_fast[], entry_ema_slow[];
   ArraySetAsSeries(entry_rates, true);
   ArraySetAsSeries(entry_ema_fast, true);
   ArraySetAsSeries(entry_ema_slow, true);
   ...
   if(CopyRates(g_symbol, InpEntryTimeframe, 0, bars_needed, entry_rates) < bars_needed)
      return false;
   if(InpUseEntryEmaAlignment)
   {
      if(CopyBuffer(g_fast_ema_entry_handle, 0, 0, 6, entry_ema_fast) < 6)
         return false;
      if(CopyBuffer(g_slow_ema_entry_handle, 0, 0, 6, entry_ema_slow) < 6)
         return false;
   }
   ...
   bool candle_buy_ok = IsTrendCandleConfirmationOk(true, entry_rates, snapshot.atr);
   bool candle_sell_ok = IsTrendCandleConfirmationOk(false, entry_rates, snapshot.atr);
   bool recent_move_ok = InpMaxIntrabarMoveAtr <= 0.0 ||
                          (recent_high - recent_low) <= snapshot.atr * InpMaxIntrabarMoveAtr;
   bool entry_up_ok = !InpUseEntryEmaAlignment ||
                      (entry_ema_fast[1] > entry_ema_slow[1] &&
                       entry_ema_fast[1] >= entry_ema_fast[3] &&
                       confirmation_close > entry_ema_fast[1]);
   bool entry_down_ok = !InpUseEntryEmaAlignment ||
                        (entry_ema_fast[1] < entry_ema_slow[1] &&
                         entry_ema_fast[1] <= entry_ema_fast[3] &&
                         confirmation_close < entry_ema_fast[1]);
   ...
   if(snapshot.regime == REGIME_TREND_UP)
   {
      ...
      bool rsi_room = snapshot.rsi < InpTrendBuyMaxRsi;
      bool structure_room_ok = IsStructureRoomOk(true, confirmation_close, snapshot.atr);
      bool structure_pullback = IsStructurePullbackEntryOk(true, entry_rates[1], snapshot.atr);
      bool fibonacci_pullback = IsFibonacciTrendPullbackOk(true, entry_rates, lookback, snapshot.atr);
      if(had_pullback && recent_move_ok && entry_up_ok && candle_buy_ok && breaks_high && bullish_bar && close_strong &&
         body_ok && bar_not_extreme && rsi_room && structure_room_ok && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_BUY;
         snapshot.reason = "Trend buy v7: candle-confirmed pullback breakout";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      if(had_pullback && recent_move_ok && entry_up_ok && candle_buy_ok && structure_pullback && bullish_bar &&
         close_strong && body_ok && bar_not_extreme && rsi_room && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_BUY;
         snapshot.reason = "Trend buy v7: candle-confirmed support continuation";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      ...
      if(InpDebugSignalLog)
         Print("Trend entry rejected BUY | time=", TimeToString(entry_rates[1].time, TIME_DATE | TIME_MINUTES),
               " hadPullback=", had_pullback,
               " recentMoveOk=", recent_move_ok,
               " entryUpOk=", entry_up_ok,
               " candleOk=", candle_buy_ok,
               " breaksHigh=", breaks_high,
               " structurePullback=", structure_pullback,
               " bullishBar=", bullish_bar,
               " closeStrong=", close_strong,
               " bodyOk=", body_ok,
               " barNotExtreme=", bar_not_extreme,
               " rsiRoom=", rsi_room,
               " structureRoom=", structure_room_ok,
               " fib=", fibonacci_pullback,
   ...
   else if(snapshot.regime == REGIME_TREND_DOWN)
   {
      ...
      bool rsi_room = snapshot.rsi > InpTrendSellMinRsi;
      bool structure_room_ok = IsStructureRoomOk(false, confirmation_close, snapshot.atr);
      bool structure_pullback = IsStructurePullbackEntryOk(false, entry_rates[1], snapshot.atr);
      bool fibonacci_pullback = IsFibonacciTrendPullbackOk(false, entry_rates, lookback, snapshot.atr);
      if(had_pullback && recent_move_ok && entry_down_ok && candle_sell_ok && breaks_low && bearish_bar && close_weak &&
         body_ok && bar_not_extreme && rsi_room && structure_room_ok && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_SELL;
         snapshot.reason = "Trend sell v7: candle-confirmed pullback breakdown";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      if(had_pullback && recent_move_ok && entry_down_ok && candle_sell_ok && structure_pullback && bearish_bar &&
         close_weak && body_ok && bar_not_extreme && rsi_room && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_SELL;
         snapshot.reason = "Trend sell v7: candle-confirmed resistance continuation";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      ...
      if(InpDebugSignalLog)
         Print("Trend entry rejected SELL | time=", TimeToString(entry_rates[1].time, TIME_DATE | TIME_MINUTES),
               " hadPullback=", had_pullback,
               " recentMoveOk=", recent_move_ok,
               " entryDownOk=", entry_down_ok,
               " candleOk=", candle_sell_ok,
               " breaksLow=", breaks_low,
               " structurePullback=", structure_pullback,
               " bearishBar=", bearish_bar,
               " closeWeak=", close_weak,
               " bodyOk=", body_ok,
               " barNotExtreme=", bar_not_extreme,
               " rsiRoom=", rsi_room,
               " structureRoom=", structure_room_ok,
                      ...
                      double band_mid, double band_upper, double band_lower)
{
   ...
   if(snapshot.regime == REGIME_TREND_UP || snapshot.regime == REGIME_TREND_DOWN)
   {
      if(is_buy)
      {
         ...
         snapshot.tp = snapshot.entry + (snapshot.entry - snapshot.sl) * EffectiveTrendRewardRisk(true);
      }
      else
      {
         ...
         snapshot.tp = snapshot.entry - (snapshot.sl - snapshot.entry) * EffectiveTrendRewardRisk(false);
      }
   }
   else if(snapshot.regime == REGIME_RANGE)
   {
...
bool CanOpenTrade(const StrategySnapshot &snapshot, string &reason)
{
   ...
   if(InpAvoidWeekendEntries && IsWeekendEntryBlocked())
   {
      reason = "Weekend entry filter";
      return false;
   }
...
void ManageRangeGrid(const StrategySnapshot &snapshot)
{
   string reason = "";
   if(!CanOpenRangeGrid(snapshot, reason))
   {
   ...
   int range_lookback = MathMax(12, InpRangeLookbackBars);
   int bars_needed = MathMax(range_lookback + 4, MathMax(InpChoppinessPeriod, InpEfficiencyPeriod) + 4);
   if(CopyRates(g_symbol, InpSignalTimeframe, 0, bars_needed, rates) < bars_needed)
   {
      g_last_reason = "Range grid: waiting for range bars";
      return;
   }

   if(CopyBuffer(g_atr_handle, 0, 0, 32, atr) < 32 || atr[1] <= 0.0)
   {
      g_last_reason = "Range grid: waiting for ATR";
      return;
   }
   ...
   double range_width_atr = range_width / MathMax(atr[1], g_point);
   double choppiness = CalculateChoppiness(rates, MathMax(2, InpChoppinessPeriod));
   double efficiency = CalculateEfficiencyRatio(rates, MathMax(2, InpEfficiencyPeriod));
   double touch_buffer = atr[1] * MathMax(0.0, InpRangeTouchBufferAtr);
   int lower_touches = CountLowerBoundaryTouches(rates, 2, range_lookback, range_low, touch_buffer);
   int upper_touches = CountUpperBoundaryTouches(rates, 2, range_lookback, range_high, touch_buffer);

   if(range_width_atr < InpRangeMinWidthAtr || range_width_atr > InpRangeGridMaxWidthAtr ||
      choppiness < InpRangeGridMinChoppiness || efficiency > InpRangeGridMaxEfficiency ||
      lower_touches < InpRangeGridMinTouches || upper_touches < InpRangeGridMinTouches)
   {
      g_last_reason = StringFormat("Range grid skipped: width %.2f ATR, chop %.1f, ER %.2f, touches %d/%d",
                                   range_width_atr, choppiness, efficiency, lower_touches, upper_touches);
      return;
   }
   ...
   double half_width = range_width * 0.5;
   double entry_zone = MathMax(0.05, MathMin(0.45, InpRangeGridEntryZone));
   double step_distance = atr[1] * MathMax(0.10, InpRangeGridStepAtr);
   int max_grid_positions = MathMax(1, InpRangeGridMaxPositions);
   int max_side_positions = MathMax(1, max_grid_positions / 2);

   int total_grid_positions = CountGridPositions();
   if(total_grid_positions >= max_grid_positions)
   {
      g_last_reason = "Range grid skipped: max grid positions reached";
      return;
   }

   int higher_bias = 0;
   if(InpRangeGridUseHigherBias)
   {
      higher_bias = CalculateRangeGridHigherBias();
      if(higher_bias == 0)
      {
         g_last_reason = "Range grid skipped: higher timeframe bias is unclear";
         return;
      }
   }

   bool buy_zone = ask <= mid - half_width * entry_zone && ask >= range_low - atr[1] * 0.15;
   bool sell_zone = bid >= mid + half_width * entry_zone && bid <= range_high + atr[1] * 0.15;

   if(buy_zone && (!InpRangeGridUseHigherBias || higher_bias > 0) &&
      InpTradeDirection != DIRECTION_SHORT_ONLY &&
      CountGridPositionsByType(POSITION_TYPE_BUY) < max_side_positions &&
      IsGridSpacingOk(POSITION_TYPE_BUY, ask, step_distance))
   {
      OpenRangeGridOrder(SIGNAL_BUY, ask, range_low, mid, atr[1], range_width_atr);
      return;
   }

   if(sell_zone && (!InpRangeGridUseHigherBias || higher_bias < 0) &&
      InpTradeDirection != DIRECTION_LONG_ONLY &&
      CountGridPositionsByType(POSITION_TYPE_SELL) < max_side_positions &&
      IsGridSpacingOk(POSITION_TYPE_SELL, bid, step_distance))
   {
      OpenRangeGridOrder(SIGNAL_SELL, bid, range_high, mid, atr[1], range_width_atr);
      return;
   }

   g_last_reason = "Range grid: waiting for price near range edge";
}

bool CanOpenRangeGrid(const StrategySnapshot &snapshot, string &reason)
{
   if(!g_auto_enabled)
   {
      reason = "Range grid blocked: auto trading is paused";
      return false;
   }

   if(snapshot.regime != REGIME_RANGE)
   {
      reason = "Range grid blocked: market is not range";
      return false;
   }
   ...
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED) || !MQLInfoInteger(MQL_TRADE_ALLOWED) ||
      !AccountInfoInteger(ACCOUNT_TRADE_ALLOWED))
   {
      reason = "Range grid blocked: trading disabled";
      return false;
   }

   if(SymbolInfoInteger(g_symbol, SYMBOL_TRADE_MODE) == SYMBOL_TRADE_MODE_DISABLED)
   {
      reason = "Range grid blocked: symbol disabled";
      return false;
   }

   if(InpUseTradeHours && !IsWithinTradeHours())
   {
      reason = "Range grid blocked: outside configured trade hours";
      return false;
   }

   if(InpAvoidWeekendEntries && IsWeekendEntryBlocked())
   {
      reason = "Range grid blocked: weekend filter";
      return false;
   }

   if(CurrentSpreadPoints() > InpMaxSpreadPoints)
   {
      reason = "Range grid blocked: spread is too high";
      return false;
   }

   if(IsDailyLossLimitHit())
   {
      reason = "Range grid blocked: daily loss limit";
      return false;
   }

   if(IsDailyTradeLimitReached())
   {
      reason = "Range grid blocked: daily trade limit";
      return false;
   }

   if(CountNonGridPositions() > 0)
   {
      reason = "Range grid blocked: non-grid position is open";
      return false;
   }
...
bool OpenRangeGridOrder(ENUM_TRADE_SIGNAL signal, double entry, double range_edge,
                        double range_mid, double atr_value, double range_width_atr)
{
   bool is_buy = signal == SIGNAL_BUY;
   double min_stop = MathMax((double)InpMinStopPoints,
                             (double)SymbolInfoInteger(g_symbol, SYMBOL_TRADE_STOPS_LEVEL) + 5.0) * g_point;
   double sl = is_buy ? range_edge - atr_value * MathMax(0.10, InpRangeGridStopBufferAtr)
                      : range_edge + atr_value * MathMax(0.10, InpRangeGridStopBufferAtr);
   double min_profit_distance = atr_value * MathMax(0.10, InpRangeGridTakeProfitAtr);
   double tp = range_mid;
   if(is_buy && tp - entry < min_profit_distance)
      tp = entry + min_profit_distance;
   else if(!is_buy && entry - tp < min_profit_distance)
      tp = entry - min_profit_distance;
   ...
   string lot_reason = "";
   double lots = CalculateOrderLots(entry, sl, lot_reason);
   if(lots <= 0.0)
   {
      g_last_reason = "Range grid blocked: " + lot_reason;
      TrackBlockedTrade(lot_reason);
      return false;
   }

   string order_reason = StringFormat("Grid %s: range %.2f ATR mean reversion",
                                      is_buy ? "buy" : "sell", range_width_atr);
   g_diag_order_attempts++;
   if(SendMarketOrder(signal, lots, sl, tp, order_reason))
   {
      g_diag_order_success++;
      g_last_trade_bar_time = iTime(g_symbol, InpSignalTimeframe, 0);
      return true;
...
int CalculateRangeGridHigherBias()
{
   double fast[], slow[];
   ArraySetAsSeries(fast, true);
   ArraySetAsSeries(slow, true);

   if(CopyBuffer(g_fast_ema_higher_handle, 0, 0, 6, fast) < 6)
      return 0;
   if(CopyBuffer(g_slow_ema_higher_handle, 0, 0, 6, slow) < 6)
      return 0;

   bool bullish = fast[1] > slow[1] &&
                  fast[1] >= fast[3] &&
                  slow[1] >= slow[3];
   bool bearish = fast[1] < slow[1] &&
                  fast[1] <= fast[3] &&
                  slow[1] <= slow[3];
...
{
   return StringFind(comment, "Grid", 0) == 0;
}
...
int CountGridPositions()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionGetString(POSITION_SYMBOL) != g_symbol ||
         ...
         continue;
      if(IsGridComment(PositionGetString(POSITION_COMMENT)))
         count++;
   }
   return count;
}

int CountGridPositionsByType(ENUM_POSITION_TYPE position_type)
{
   int count = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
...
bool IsGridSpacingOk(ENUM_POSITION_TYPE position_type, double entry_price, double min_distance)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      ...
      if(MathAbs(open_price - entry_price) < min_distance)
         return false;
   }
...
bool IsTrendCandleConfirmationOk(bool is_buy_signal, MqlRates &rates[], double atr_value)
{
   if(!InpUseCandleTrendFilter)
      return true;
   if(ArraySize(rates) < 6 || atr_value <= 0.0)
      return true;

   double bar_range = MathMax(rates[1].high - rates[1].low, g_point);
   double body = MathAbs(rates[1].close - rates[1].open);
   if(body < atr_value * MathMax(0.0, InpCandleMinBodyAtr))
      return false;
   ...
   double opposite_wick = is_buy_signal ? upper_wick : lower_wick;
   if(opposite_wick / bar_range > MathMax(0.0, InpCandleMaxOppositeWickRatio))
      return false;

   int lookback = MathMax(1, InpCandleMomentumLookback);
   int required = MathMax(0, MathMin(lookback, InpCandleMinMomentumCloses));
   if(required <= 0)
      return true;

   int momentum_closes = 0;
   int max_index = MathMin(lookback, ArraySize(rates) - 2);
   for(int i = 1; i <= max_index; i++)
   {
      if(is_buy_signal && rates[i].close > rates[i + 1].close)
         momentum_closes++;
      else if(!is_buy_signal && rates[i].close < rates[i + 1].close)
         momentum_closes++;
   }
...
bool IsStructureRoomOk(bool is_buy_signal, double price, double atr_value)
{
   if(!InpUseStructureRoomFilter)
      return true;
   if(atr_value <= 0.0)
      return true;
   ...
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int lookback = MathMax(12, InpStructureLookbackBars);
   int span = MathMax(1, InpStructurePivotSpan);
   int bars_needed = lookback + span + 6;
   if(CopyRates(g_symbol, InpSignalTimeframe, 0, bars_needed, rates) < bars_needed)
      return true;

   double break_buffer = atr_value * MathMax(0.0, InpStructureBreakBufferAtr);
   double min_room = atr_value * MathMax(0.0, InpMinRoomToStructureAtr);

   if(is_buy_signal)
   {
      double resistance = FindNearestSwingHighAbove(rates, price + break_buffer, 2, lookback, span);
      if(resistance <= 0.0)
         return true;
      return resistance - price >= min_room;
   }

   double support = FindNearestSwingLowBelow(rates, price - break_buffer, 2, lookback, span);
   if(support <= 0.0)
      return true;
   return price - support >= min_room;
}

bool IsStructurePullbackEntryOk(bool is_buy_signal, const MqlRates &entry_bar, double atr_value)
{
   if(!InpUseStructurePullbackEntry)
      return false;
   if(atr_value <= 0.0)
      return false;
   ...
   double zone = atr_value * MathMax(0.0, InpStructurePullbackZoneAtr);
   double reject_buffer = atr_value * MathMax(0.0, InpStructureRejectBufferAtr);

   if(is_buy_signal)
   {
      double support = FindNearestSwingLowBelow(rates, entry_bar.low + zone, 2, lookback, span);
      if(support <= 0.0)
         return false;
      bool tested_support = entry_bar.low <= support + zone;
      bool closed_back = entry_bar.close >= support + reject_buffer;
      return tested_support && closed_back;
   }

   double resistance = FindNearestSwingHighAbove(rates, entry_bar.high - zone, 2, lookback, span);
   if(resistance <= 0.0)
      return false;
   bool tested_resistance = entry_bar.high >= resistance - zone;
   bool closed_back = entry_bar.close <= resistance - reject_buffer;
   return tested_resistance && closed_back;
}

bool IsFibonacciTrendPullbackOk(bool is_buy_signal, MqlRates &entry_rates[],
                                int entry_lookback, double atr_value)
{
   if(!InpUseFibonacciTrendFilter)
      return true;
   if(is_buy_signal && !InpUseFibonacciForLong)
      return true;
   if(!is_buy_signal && !InpUseFibonacciForShort)
      return true;
   if(atr_value <= 0.0)
      return true;
   ...
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int lookback = MathMax(20, InpFibonacciLookbackBars);
   int bars_needed = lookback + 8;
   if(CopyRates(g_symbol, InpSignalTimeframe, 0, bars_needed, rates) < bars_needed)
      return true;

   double min_ratio = MathMax(0.05, MathMin(0.95, InpFibonacciMinRetracement));
   double max_ratio = MathMax(min_ratio, MathMin(0.95, InpFibonacciMaxRetracement));
   double buffer = atr_value * MathMax(0.0, InpFibonacciZoneBufferAtr);
   int usable_entry_lookback = MathMax(2, entry_lookback);

   if(is_buy_signal)
   {
      int high_index = HighestHighIndex(rates, 2, lookback);
      if(high_index < 0)
         return true;
      int low_index = LowestLowIndex(rates, high_index + 1, lookback - high_index);
      if(low_index < 0)
         return true;

      double impulse_low = rates[low_index].low;
      double impulse_high = rates[high_index].high;
      double impulse = impulse_high - impulse_low;
      if(impulse < atr_value * MathMax(0.10, InpFibonacciMinImpulseAtr))
         return false;

      double zone_high = impulse_high - impulse * min_ratio;
      double zone_low = impulse_high - impulse * max_ratio;
      double pullback_low = LowestLow(entry_rates, 1, usable_entry_lookback);
      return pullback_low <= zone_high + buffer && pullback_low >= zone_low - buffer;
   }

   int low_index = LowestLowIndex(rates, 2, lookback);
   if(low_index < 0)
      return true;
   int high_index = HighestHighIndex(rates, low_index + 1, lookback - low_index);
   if(high_index < 0)
      return true;
   ...
   double zone_low = impulse_low + impulse * min_ratio;
   double zone_high = impulse_low + impulse * max_ratio;
   double pullback_high = HighestHigh(entry_rates, 1, usable_entry_lookback);
   return pullback_high >= zone_low - buffer && pullback_high <= zone_high + buffer;
}

double FindNearestSwingHighAbove(MqlRates &rates[], double price, int start_index, int count, int span)
{
   double nearest = 0.0;
   int end_index = MathMin(start_index + count, ArraySize(rates) - span);
   for(int i = MathMax(start_index, span); i < end_index; i++)
   {
      if(rates[i].high < price)
         continue;
      if(!IsSwingHigh(rates, i, span))
         continue;
      if(nearest <= 0.0 || rates[i].high < nearest)
         nearest = rates[i].high;
   }
   return nearest;
}

double FindNearestSwingLowBelow(MqlRates &rates[], double price, int start_index, int count, int span)
{
   double nearest = 0.0;
   int end_index = MathMin(start_index + count, ArraySize(rates) - span);
   for(int i = MathMax(start_index, span); i < end_index; i++)
   {
      ...
      if(!IsSwingLow(rates, i, span))
         continue;
      if(nearest <= 0.0 || rates[i].low > nearest)
         nearest = rates[i].low;
   }
   return nearest;
}

bool IsSwingHigh(MqlRates &rates[], int index, int span)
{
   if(index - span < 0 || index + span >= ArraySize(rates))
      return false;
...
bool IsSwingLow(MqlRates &rates[], int index, int span)
{
   if(index - span < 0 || index + span >= ArraySize(rates))
      return false;
...
void CloseAllManagedPositions()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      ClosePositionMarket(ticket, "Panel close");
   }

   g_last_trade_result = "Close command sent";
}

bool ClosePositionMarket(ulong ticket, string comment)
{
   if(ticket == 0 || !PositionSelectByTicket(ticket))
      return false;
...
{
   if(!InpUseBreakevenStop && !InpUseAtrTrailingStop && InpMaxLossCloseR <= 0.0)
      return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double risk_distance = EstimatePositionInitialRisk(open_price, current_sl, current_tp, is_buy_position);
      if(risk_distance <= 0.0)
         continue;

      double profit_distance = is_buy_position ? bid - open_price : open_price - ask;
      double r_multiple = profit_distance / risk_distance;
      if(InpMaxLossCloseR > 0.0 && r_multiple <= -InpMaxLossCloseR)
      {
         if(ClosePositionMarket(ticket, "Max loss R close"))
            g_last_trade_result = "Closed: max loss R";
         continue;
      }
...
double EstimatePositionInitialRisk(double open_price, double current_sl, double current_tp, bool is_buy_position)
{
   double risk_from_target = 0.0;
   if(current_tp > 0.0)
      risk_from_target = MathAbs(current_tp - open_price) / MathMax(0.10, EffectiveTrendRewardRisk(is_buy_position));
...
int CalculateDealHoldingSeconds(ulong closing_deal_ticket)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      if(deal_ticket == 0 || deal_ticket == closing_deal_ticket)
         continue;
   ...
   return (int)(close_time - open_time);
}
         ...
         if(entry_type == DEAL_ENTRY_OUT || entry_type == DEAL_ENTRY_INOUT)
         {
            ...
            if(deal_profit < 0.0)
            {
               ...
               if(deal_time > g_last_loss_deal_time)
               {
                  ...
                  g_last_loss_hold_seconds = CalculateDealHoldingSeconds(deal_ticket);
               }
            }
         }
      }
...
bool IsDailyTradeLimitReached()
{
   ...
   if(!InpAllowRecoveryTradeAfterDailyLoss || InpMaxDailyRecoveryTrades <= 0)
      return true;

   if(g_daily_profit >= 0.0)
      return true;

   bool quality_filter_configured = InpMinDailyLossForRecoveryTrade > 0.0 ||
                                    InpMinRecoveryLossHoldMinutes > 0;
   if(quality_filter_configured)
   {
      bool loss_size_ok = InpMinDailyLossForRecoveryTrade > 0.0 &&
                          g_daily_profit <= -InpMinDailyLossForRecoveryTrade;
      bool loss_hold_ok = InpMinRecoveryLossHoldMinutes > 0 &&
                          g_last_loss_hold_seconds >= InpMinRecoveryLossHoldMinutes * 60;

      if(!loss_size_ok && !loss_hold_ok)
         return true;
   }

   return g_daily_trades >= InpMaxTradesPerDay + InpMaxDailyRecoveryTrades;
}
...
bool IsWeekendEntryBlocked()
{
   ...
   int friday_last_hour = MathMax(0, MathMin(23, InpFridayLastEntryHour));
   if(current.day_of_week == 5 && current.hour >= friday_last_hour)
      return true;

   if(current.day_of_week == 6 || current.day_of_week == 0)
      return true;
...
double EffectiveTrendRewardRisk(bool is_buy_signal)
{
   double configured_rr = InpTrendRewardRisk;
   if(is_buy_signal && InpLongTrendRewardRisk > 0.0)
      configured_rr = InpLongTrendRewardRisk;
   else if(!is_buy_signal && InpShortTrendRewardRisk > 0.0)
      configured_rr = InpShortTrendRewardRisk;

   if(g_profile_name == "Conservative")
      return MathMax(1.50, configured_rr);
   if(g_profile_name == "Aggressive")
      return MathMax(1.30, configured_rr * 0.90);
   return configured_rr;
}
...
int CountLowerBoundaryTouches(MqlRates &rates[], int start_index, int count, double boundary, double buffer)
{
   int touches = 0;
   int last_touch_index = -1000;
   int end_index = MathMin(start_index + count, ArraySize(rates));

   for(int i = start_index; i < end_index; i++)
   {
      if(rates[i].low <= boundary + buffer && i - last_touch_index >= 2)
      {
         touches++;
         last_touch_index = i;
      }
...
int CountUpperBoundaryTouches(MqlRates &rates[], int start_index, int count, double boundary, double buffer)
{
   int touches = 0;
   int last_touch_index = -1000;
   int end_index = MathMin(start_index + count, ArraySize(rates));
   ...
   for(int i = start_index; i < end_index; i++)
   {
      if(rates[i].high >= boundary - buffer && i - last_touch_index >= 2)
      {
         touches++;
         last_touch_index = i;
      }
...
int HighestHighIndex(MqlRates &rates[], int start_index, int count)
{
   if(start_index < 0 || start_index >= ArraySize(rates))
      return -1;
...
int LowestLowIndex(MqlRates &rates[], int start_index, int count)
{
   if(start_index < 0 || start_index >= ArraySize(rates))
      return -1;
