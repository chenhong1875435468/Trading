...
input int                  InpMaxStopPoints            = 3500;
...
bool BuildStrategySnapshot(StrategySnapshot &snapshot)
{
   ...
   bool trend_quality_ok = !InpUseAdvancedFilters ||
   ...
   if(adx[1] >= InpTrendAdxThreshold && atr_active && trend_quality_ok &&
      ema_up && higher_up && close_above_mid && di_bullish)
      snapshot.regime = REGIME_TREND_UP;
   else if(adx[1] >= InpTrendAdxThreshold && atr_active && trend_quality_ok &&
