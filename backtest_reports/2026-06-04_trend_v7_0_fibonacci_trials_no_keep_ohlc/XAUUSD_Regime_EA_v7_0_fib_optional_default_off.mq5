...
bool BuildIntrabarTrendSignal(StrategySnapshot &snapshot, bool is_new_entry_bar)
{
   ...
   if(snapshot.regime == REGIME_TREND_UP)
   {
      ...
      if(had_pullback && recent_move_ok && entry_up_ok && breaks_high && bullish_bar && close_strong &&
         body_ok && bar_not_extreme && rsi_room && structure_room_ok && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_BUY;
         snapshot.reason = "Trend buy v6: Fibonacci pullback breakout";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      if(had_pullback && recent_move_ok && entry_up_ok && structure_pullback && bullish_bar &&
         close_strong && body_ok && bar_not_extreme && rsi_room && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_BUY;
         snapshot.reason = "Trend buy v5: support pullback continuation";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
   ...
   else if(snapshot.regime == REGIME_TREND_DOWN)
   {
      ...
      if(had_pullback && recent_move_ok && entry_down_ok && breaks_low && bearish_bar && close_weak &&
         body_ok && bar_not_extreme && rsi_room && structure_room_ok && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_SELL;
         snapshot.reason = "Trend sell v6: Fibonacci pullback breakdown";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
      if(had_pullback && recent_move_ok && entry_down_ok && structure_pullback && bearish_bar &&
         close_weak && body_ok && bar_not_extreme && rsi_room && fibonacci_pullback)
      {
         snapshot.signal = SIGNAL_SELL;
         snapshot.reason = "Trend sell v5: resistance pullback continuation";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
