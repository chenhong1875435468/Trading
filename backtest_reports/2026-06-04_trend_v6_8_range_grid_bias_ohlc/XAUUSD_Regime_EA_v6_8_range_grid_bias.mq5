...
bool BuildIntrabarTrendSignal(StrategySnapshot &snapshot, bool is_new_entry_bar)
{
   ...
   if(snapshot.regime == REGIME_TREND_UP)
   {
      ...
      if(had_pullback && recent_move_ok && entry_up_ok && breaks_high && bullish_bar && close_strong && body_ok && bar_not_extreme && rsi_room)
      {
         snapshot.signal = SIGNAL_BUY;
         snapshot.reason = "Trend buy v4: M5 EMA-aligned breakout in strong uptrend";
   ...
   else if(snapshot.regime == REGIME_TREND_DOWN)
   {
      ...
      if(had_pullback && recent_move_ok && entry_down_ok && breaks_low && bearish_bar && close_weak && body_ok && bar_not_extreme && rsi_room)
      {
         snapshot.signal = SIGNAL_SELL;
         snapshot.reason = "Trend sell v4: M5 EMA-aligned breakdown in strong downtrend";
         BuildTradePricesFromMarket(snapshot);
         g_last_intrabar_signal_time = entry_bar_time;
         return true;
      }
