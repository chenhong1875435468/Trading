...
input double               InpMaxActualRiskPercent     = 8.00;
...
input double               InpRangeRewardRisk          = 0.75;
...
input int                  InpMaxStopPoints            = 12000;

input int                  InpMaxSpreadPoints          = 350;
input int                  InpMaxSlippagePoints        = 30;
input int                  InpMaxOpenPositions         = 1;
input int                  InpMaxTradesPerDay          = 3;
input double               InpMaxDailyLossPercent      = 6.0;
input double               InpMaxDailyLossMoney        = 6.0;
...
input double               InpAtrTrailTriggerR         = 1.60;
input double               InpAtrTrailMultiplier       = 1.40;
input int                  InpMinManagedStopStepPoints = 500;
...
bool BuildStrategySnapshot(StrategySnapshot &snapshot)
{
   ...
   bool di_bullish = plus_di[1] > minus_di[1] && plus_di[1] >= plus_di[2];
   bool di_bearish = minus_di[1] > plus_di[1] && minus_di[1] >= minus_di[2];
   ...
   if(adx[1] >= InpTrendAdxThreshold && atr_active && ema_up && higher_up && close_above_mid && di_bullish)
      snapshot.regime = REGIME_TREND_UP;
   else if(adx[1] >= InpTrendAdxThreshold && atr_active && ema_down && higher_down && close_below_mid && di_bearish)
   ...
   if(snapshot.regime == REGIME_TREND_UP)
   {
      ...
      else if(pullback && resumed && rsi_room)
      {
         signal = SIGNAL_BUY;
   ...
   else if(snapshot.regime == REGIME_RANGE)
   {
      ...
      {
      bool near_lower = rates[1].low <= bands_lower[1] + atr[1] * 0.35 && rates[1].close > bands_lower[1] - atr[1] * 0.10;
      bool near_upper = rates[1].high >= bands_upper[1] - atr[1] * 0.35 && rates[1].close < bands_upper[1] + atr[1] * 0.10;
...
bool BuildIntrabarTrendSignal(StrategySnapshot &snapshot, bool is_new_entry_bar)
{
   ...
   if(snapshot.regime == REGIME_TREND_UP)
   {
      ...
      bool rsi_room = snapshot.rsi < 72.0;
      if(had_pullback && breaks_high && bullish_bar && close_strong && body_ok && bar_not_extreme && rsi_room)
      {
   ...
   else if(snapshot.regime == REGIME_TREND_DOWN)
   {
      ...
      if(had_pullback && breaks_low && bearish_bar && close_weak && body_ok && bar_not_extreme && rsi_room)
      {
